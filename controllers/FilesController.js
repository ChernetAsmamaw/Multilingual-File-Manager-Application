...
// Convert fs.unlink to a promise-based function - this is a workaround for the error "TypeError: fs.unlink is not a function"
// unlink is used in the deleteFile method below to delete files from the file system
const unlink = promisify(fs.unlink);

// File controller class for handling file management
export default class FilesController {
  ...
  // Method to publish a file: make it public
  ...
  // Method to delete a file from the database and the file system
  static async deleteFile(req, res) {
    ...
    try {
      ...
      await (await dbClient.filesCollection()).deleteOne(fileFilter);

      // If it's not a folder, delete the actual file from the file system
      if (file.type !== VALID_FILE_TYPES.folder) {
        if (existsSync(file.localPath)) {
          try {
            await unlink(file.localPath); // Wait for file deletion to complete
          } catch (err) {
            console.error('Error deleting file:', err);
            return res.status(500).json({ error: 'Error deleting file from the file system' });
          }
        }
      }
